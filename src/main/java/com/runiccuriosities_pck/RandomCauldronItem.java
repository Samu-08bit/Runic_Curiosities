...
public class RandomCauldronItem extends TalismanItem implements ICurioItem {
    ...
    // 3 minuti in ticks (20 ticks * 60 secondi * 3 = 3600)
    ...
    public void curioTick(SlotContext slotContext, ItemStack stack) {
        ...
        // 1.21.1: Leggiamo i CustomData invece degli NBT
        CustomData customData = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY);
        CompoundTag tag = customData.copyTag();
        int timer = tag.contains("CauldronTimer") ? tag.getInt("CauldronTimer") : 0;

        // Se il talismano è scarico (durabilità a 0 = danno a 1)
    ...
    private boolean consumeGhastTear(Player player) {
        for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
            ...
            if (invStack.is(Items.GHAST_TEAR)) {
                invStack.shrink(1);
                return true;
            }
        }
    ...
    private void applyRandomPositiveEffect(Player player) {
        // 1.21.1: I MobEffects ora restituiscono degli Holder<MobEffect>
        ...
        // Assegna l'effetto randomico per 3 minuti (3600 ticks)
    ...
    public int getBarWidth(ItemStack stack) {
        if (stack.getDamageValue() >= 1) {
            return 0; // Se scarico, barra vuota
        }

        CustomData customData = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY);
        int timer = customData.contains("CauldronTimer") ? customData.copyTag().getInt("CauldronTimer") : 0;

        // Larghezza massima della barra 13
